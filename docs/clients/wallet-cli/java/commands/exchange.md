...
- `expected`——你愿意接受的**另一种 token 的最低到手数量**，以该 token 的最小单位计。它是一个滑点下限，而不是价格：链会沿该交易对的联合曲线算出真实产出，若低于 `expected` 就拒绝这笔交易。它与 `quant` 计价的币种不同，因此两者不可比较——`expected` **并不要求**小于 `quant`。它必须大于 0。
...
把 100 个最小单位的 token 1000001 卖入 1 号交易对，并要求至少换回该交易对另一种 token 的 80 个最小单位。如果按当时的曲线换回的不足 80，交易就会失败，什么也不会卖出。这里的 `80` 是到手数量的下限，而不是价格。
