...
创建一条仅观察记录；本地不保存任何密钥。要求设备已解锁并打开所选的 app。
...
`--app` 正是让 Ledger 账户成为**单家族**账户的原因：TRON app 注册出的是 `tron` 账户，以太坊 app 注册出的是 `evm` 账户，得到的账户只有那一个地址。想同时持有两个，就用两个 app 各导入这台设备一次。参见 [Ledger 指南](../../guide/ledger.md)。
