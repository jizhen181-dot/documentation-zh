...
1. **验证**——输入当前的 master password；它必须能解密某个已有的 keystore（否则 `auth_failed`，且不会改动任何东西）。
2. **设置**——输入两次新密码。两次不一致或不满足强度策略时，会在提示处被拒并要求重输。
3. **确认**——命令会列出将要重新加密多少个软件钱包；`[y/N]`（加 `--yes` 可跳过）。选择否则中止，不做任何更改。
4. **原子地重新加密**——对每个 keystore：用旧密码解密 → 用新密码加密 → 写入临时文件 → fsync；只有在*全部*成功之后，才把这些文件改名就位。任一步失败都会整体回滚并报告 `io_error`——原有的 keystore 仍然有效。
...
本命令是交互式的。text 模式下，回执会列出重新加密过的软件钱包，且绝不包含任何密钥材料。JSON 模式下，`data.wallets` 给出这些钱包的标签/id，`data.count` 给出数量。本地命令——没有 `chain` 块。
