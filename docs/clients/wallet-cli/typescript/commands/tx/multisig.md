...
> `tx multisig` 只能通过外部的 TronLink 多签服务工作，并且需要凭据（`tronlinkSecretId` / `tronlinkSecretKey` / `tronlinkChannel`）。它只是一层可选的便利封装——链上路径（[`tx sign`](sign.md) / [`tx approvals`](approvals.md) / [`tx broadcast`](broadcast.md)）不依赖任何服务也能完成同样的事。没有凭据时本命令不可用（`tronlink_credentials_missing`）。
...
> **仅限 TRON。** EVM 交易只带一个签名，因此没有阈值需要达到，也没有什么可收集的；本命令在 EVM 网络上会以 `family_mismatch` 失败。

链上路径是把 hex 在人与人之间传递，而服务路径则由 TronLink 服务**保管**交易、逐个**累计**签名，并通过 WebSocket 向联署人**推送**通知。该命令有四种互斥的模式：

- **默认（不带模式参数）**——列出服务上与该账户相关的多签交易及其进度。这是日常查看「有什么在等你」的方式。
- **`--create`**——在本地对一笔**未签名**的交易签名并提交，从而开启一轮收集。输入是未签名的 hex，由任意广播命令的 `--build-only` 模式产出（例如 `tx send … --build-only`）。软件账户需要 master password；Ledger 账户在设备上确认。
- **`--sign <txId>`**——联署其中一笔：取回它以及此前已收集到的签名，在本地签名，再把整笔交易提交回去，由服务继续累积。软件账户需要 master password；Ledger 账户在设备上确认。
- **`--watch`**——保持一个 WebSocket 连接，用**数量**提醒你有多少笔交易在等你签名（不含明细）；要处理它们，请用默认模式列出。
...
| 字段 | 类型 | 含义 |
|---|---|---|
| `address` | string | 读取队列时所针对的账户 |
| `total` | number | 服务报告的交易数量 |
| `unreadable` | number | 因本客户端无法解码而被略去的记录数 |
| `transactions[].txId` | string | 交易 id |
| `transactions[].state` | string | `pending` \| `signed` \| `success` \| `failed` |
| `transactions[].verified` | boolean | 该记录是否与链上对上了 |
| `transactions[].unverifiedReason` | string? | 仅当 `verified` 为 `false` 时才有 |
| `transactions[].contractType` | string | 服务报告的合约类型 |
| `transactions[].from` / `to` | string? | 解码出的发送方与收款方，仅当该合约类型会暴露它们时 |
| `transactions[].rawAmount` | string? | 可得时给出解码出的原始整数金额；单位随合约类型而定 |
| `transactions[].createdAt` / `expiration` | number | 服务侧的创建时间与交易过期时间，均为 Unix 毫秒 |
| `transactions[].expired` | boolean | 该交易是否已经过期 |
| `transactions[].signatures` | number | 当前已附上的签名数 |
| `transactions[].signatureProgress` | array | 逐个密钥的 `address`、`weight`、`signed`，以及可为空的 `signedAt` |
...
`--watch` 会持续推送计数提示。停止时，它在 JSON 模式下的终态结果是 `{action: "watch", address, notifications}`；text 模式则打印同样的地址和通知计数。

## 退出码

`0` 成功 · `1` 执行失败（`not_found`——服务上没有该 txId、`not_authorized`、`already_signed`、`tx_expired`、`auth_failed`、`provider_error`——服务出错或限流；TronLink 会把**所有**非 404 的状态都归到这一类，包括 429） · `2` 用法错误（`tronlink_credentials_missing`、`unsupported_network`、`invalid_value`——包括把已签名的交易传给 `--create`、模式互相冲突）。
