...
所选网络决定**用哪把密钥签名、报告哪个地址**：一个账户在每个家族下各有一把密钥，因此 `--network nile` 用它的 TRON 密钥签名并报告 base58 地址，`--network sepolia` 用它的 EVM 密钥签名并报告 `0x` 地址。`--network` 是可选的，省略时回落到 `config.defaultNetwork`；无论如何，签名本身完全是离线完成的。
...
同一条消息在 EVM 网络上签名会得到不同的签名，因为用的是另一把密钥：
